...
std::unique_ptr<Command> DeleteReplStep::execute(Context ctx) {
  --stage_;
  if (stage_ == 1) {
...
void DeleteReplStep::ChangeStep(std::shared_ptr<ReplSteps> &active_step) {
  if (stage_ == 0) {
...
template <>
std::unique_ptr<Command> DeleteReplStep::HandleStage<1>(Context &) {
  if (arg_.empty()) {
...
template <>
std::unique_ptr<Command> DeleteReplStep::HandleStage<0>(Context &ctx) {
  if (!ctx.solid_tasks &&
...
std::unique_ptr<Command> DeleteReplStep::ReportError(std::string str) {
  stage_ = 0;
