...
struct IostreamStrings
{
  ...
  static constexpr ccp kInvalidState = "State should be '+' for completed or '-' for uncompleted\n";
  static constexpr ccp kInvalidPriority = "Priority should be: low, medium, high.\n";
  ...
  static constexpr ccp kUnknownCommand = "Unknown command. Type 'help' to display all available commands.\n";
  static constexpr ccp kInvalidId = "Id should be numeric value.\n";
  static constexpr ccp kMultipleId = "Multiple Id does not supported for this command.\n";
  ...
  static std::string AndItsChildren(int n);

  static std::string LeaveEmptyFor(const std::string& str);

  static std::string LeaveEmptyFor(const std::string& str1, const std::string& str2);

  static std::string GetPrompt(const std::string &str);

  static std::string GetPrompt(const std::string &str1, const std::string &str2);

  static std::string ProceedTo(const std::string& str);

  std::string ShowTask(const Task&);
  std::string ShowTask(const Task&, int);
  std::string ShowTaskWithId(const Task&, TaskId);
  std::string ShowTaskWithId(const Task&, TaskId, int);

  const char* to_string(Task::Priority priority);
  const char* to_string(Task::State state);

  std::string ShowNestedMap(const TaskWrapper& task_wrapper, int nest);
};
